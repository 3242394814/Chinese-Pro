...
local filters = { "熔炉", "秋天白天", "秋天黄昏", "秋天夜晚", "雪天白天", "雪天黄昏", "雪天夜晚", "夏天白天", "夏天黄昏", "夏天夜晚", "春天白天", "春天黄昏", "春天夜晚", "疯狂的白天", "疯狂的黄昏", "疯狂的夜晚", "紫色月亮", "海难-温和季-白天(SW Mild Day)", "海难-雨天-白天(SW Wet Day)", "海难-雨季-白天(SW Green Day)", "海难-火山", "海狸", "洞穴", "真菌(Fungus)", "幽灵", "本身(Identity)", "鼹鼠视野 关闭", "鼹鼠视野 开启", "废墟-黑暗(Ruins Dark)", "废墟-昏暗(Ruins Dim)", "废墟-灯（Ruins Light）", "Sinkhole", "暴食" }
local filter_list = {}
for i = 1, #filters do
...
local info={
    ...
    AddCustomConfig("DISPLAY_TARGET_BADGE", "显示目标奖章", "显示当前目标奖章。\nDisplay current targets badge.", BuildBooleanConfig(), true),
    AddCustomConfig("PING_KEYBIND", "Ping位置", "指定要ping一个位置的键。", keylist, "KEY_R"),
    AddCustomConfig("PING_TRANSPARENCY", "Ping透明度", "影响ping横幅的透明度。\nAffects the transparency of ping banners.", BuildNumConfig(5, 100, 5, true), 100),
    ...
    AddCustomConfig("DAMAGE_NUMBER_PLAYERS", "受到的伤害数值", "显示对玩家造成的伤害数字。\nDisplay damage numbers for damage done to players.", BuildBooleanConfig({ off = "关", on = "开" }), false),
    ...
    AddCustomConfig("EVENT_TRACKING", "活动区域", "允许某些竞技场根据当前事件进行更改。\nAllows certain arenas to change depending on the current event.", BuildBooleanConfig({ off = "关", on = "开" }), true),
    ...
    AddCustomConfig("DISPLAY_COLORED_STATS", "显示彩色统计信息", "根据您的团队统计排名，为统计文本涂上金色、银色、青铜色或白色。", BuildBooleanConfig(), true),
    AddCustomConfig("DEFAULT_ROTATION", "Mvp徽章默认旋转:\nMvp Badge default rotation", "MVP徽章在默认摘要屏幕和详细摘要屏幕上具有相同的旋转\nThe MVP badge will have the same rotation on both default summary screen and the detailed summary screen", BuildBooleanConfig(), false),
    ...
    AddCustomConfig("WAVESET", "进攻波设置", "将波数据更改为新的数据!\nChange the wave data to something NEW!", wave_sets, "swineclops"),
    ...
    AddCustomConfig("BATTLESTANDARD_EFFICIENCY", "Battlestandard Effectiveness", "修改战斗标准的有效性。\nModify how effective battlestandards are.", BuildNumConfig(50, 200, 25, true), 1),
    ...
    AddCustomConfig("SANDBOX", "启用沙盒", "将沙盒进攻波添加到进攻波列表中。\nAdds the sandbox waveset to the wavesets list.", BuildBooleanConfig({ off = "关", on = "开" }), false),
