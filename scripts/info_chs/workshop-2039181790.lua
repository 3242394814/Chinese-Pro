...
local info=
{
	...
	configuration_options =
	{
		...
		Header("Client-Side","客户端"),

		BinaryConfig("um_music", "永不妥协BGM", "如果您在使用客户端音乐模式时崩溃，请禁用此选项。",true),
		BinaryConfig("um_storms_over", "龙卷风:降低视觉特效","降低龙卷风接近时的雨水视觉效果的整体强度。", false),

		Header("Mod Compatibility","模组兼容性"),

		BinaryConfig("hungry_void", "禁止虚空行走",
			"防止洞穴卡海什么的，如果你正在使用任何允许飞行或穿越洞穴空隙的MOD，请禁用此功能。",true),
		BinaryConfig("nofishyincrockpot", "没有活鱼食谱", "如果某个模组需要活鱼来制作某些食谱，请禁用此功能。",true),
		...
		BinaryConfig("maxhungerdamage", "饥饿损伤最大生命值", "饥饿会在短暂的延迟后造成损失血量上限，也就是黑血。",true),
		...
		-- 	true),
		{
			...
			options =
			{
				...
				{ description = "疯狂", data = 0 },
			},
			default = 0.8,
		},
		BinaryConfig("compromising_vortex", "非致命暗影漩涡", "暗影漩涡现在会将你传送到某个随机的地方。",false),
		BinaryConfig("harder_shadows", "更难的暗影生物", "精神值过低会出现三种强大的新影怪，请注意自己的精神值",true),
		BinaryConfig("rat_raids", "鼠潮", "老鼠会定期攻击你的基地", true),
		BinaryConfig("durability", "衣物变化", "衣物会随着耐久值下降而效果下降，比如眼球伞并不会一直100%防雨了",false),
		BinaryConfig("sewingkit", "缝纫包改动", "缝纫包能修复的耐久降低，但是能修复更多次", true),
		BinaryConfig("lifeamulet", "重生护符改动", "只有在佩戴重生护符死亡时才能复活，同时饥饿转化生命值的速度更快了",true),
		BinaryConfig("longpig", "人肉(长猪肉)", "开启后告密的心需要长猪肉才能制作，长猪肉敲骨架获得",true),
		{
			name = "flingo_setting",
			label = "灭火器改动",
			hover = "选择你想要的灭火器形式",
		...
		-- Character Reworks --
		------------------------------
		Header("Characters","人物改动"),
		------------------------------
		BinaryConfig("wixie_walter", "Wixie & 沃尔特重做", "启用永不妥协的 Wixie，犯罪者。这位不法分子将沃尔特的小弹弓加以扩展，而沃尔特则与沃比有了新的互动和机制！", true),
		BinaryConfig("funny rat", "老鼠人Winky", "启用永不妥协的Winky，卑鄙的害虫。", true),
		--BinaryConfig("wixie_birds", "Wixie:弹弓削弱", "弹弓无法击中鸟和兔子。", true),
		BinaryConfig("holy fucking shit it's wathom", "Wathom", "启用永不妥协的Wathom，被遗忘的模仿者。", true),
		BinaryConfig("wathom_max_dmg", "Wathom: 伤害上限", "wathom的伤害上限为600 以限制他荒谬的爆发伤害潜力。", true),
		{
		...
		BinaryConfig("warly_food_taste_", "沃利:食物的味道", "沃利可以从食物中获得更多的属性，就像单人游戏一样。 然而，他能记住三天的食物，而不是两天。",true),
		BinaryConfig("warly_butcher_", "沃利:屠宰",
		...
		BinaryConfig("wolfgang", "大力士", "沃尔夫冈根据饥饿等级获得力量。持续时间越长，饥饿流失越多。",
			false),
		BinaryConfig("winonaworker", "女工:更快的工作", "薇诺娜现在根据她的饥饿等级来调整她的工作/采摘效率，以及工具/武器的耐用性。采取行动时减少饥饿值",true),
		...
		BinaryConfig("applied horticulture", "老奶奶:简编园艺学", "现在需要1片叶子肉而不是5颗种子",true),
		BinaryConfig("lunar grimoire", "老奶奶:月相魔典", "\"月相魔典\" 现在只会影响你的周围。", true),
		BinaryConfig("apicultural notes", "老奶奶:养蜂笔记", "\"养蜂笔记\" 现在让你周围的蜂箱被填满。",true),
		BinaryConfig("the angler", "老奶奶:垂钓者生存指南","“钓鱼者的生存指南”现在需要2个硬物浮标，而不是2个木球浮标。", true),
		...
		BinaryConfig("wormwood_extrafiredmg", "植物人:火焰伤害", "植物人受到的火焰伤害由1.25倍提高至1.75倍",true),
		BinaryConfig("wormwood_trapbuffs", "植物人: 荆棘陷阱buff","荆棘陷阱不会对玩家造成伤害，当植物人开花时会自动重置陷阱，并调整为一次制作时收获多个。",true),
		BinaryConfig("wormwood_photosynthesis", "植物人: 光合作用", "光合作用现在允许沃姆伍德在夏天自然开花，而非缓慢恢复生命值", true),
		BinaryConfig("horticulture, expanded", "老奶奶:园艺学扩展版","现在能作用 20 株植物，而不是 15 株。", true),
		BinaryConfig("lux aeterna", "老奶奶:永恒之光之复兴","永恒之光 和 永恒之光之复兴 现在持续更长时间。", true),
		...
		BinaryConfig("snowstorms", "暴风雪", "如果玩家不戴护眼，暴风雪会影响他们的速度和视力。暴风雪也会导致雪在建筑物上堆积。",true),
		BinaryConfig("winter_burning", "冬季更难起火", "冬天使它燃烧起来需要更多的时间，并且燃烧得更快。",true),
		BinaryConfig("heatwaves", "热浪",
			"热浪是夏季暴风雪的对应物，虽然它们本身并没有太大作用，但除了温度升高之外，它们与烟雾和荨麻相互作用。",true),
		...
		BinaryConfig("hotcaves", "过热洞穴", "在夏天，洞穴的温度足以让你在没有任何装备的情况下过热。",true),
		BinaryConfig("pyrenettles", "荨麻丛", "柴堆荨麻是一种新的入侵植物，随着超高的温度生长。",true),

		BinaryConfig("smog", "烟雾",
			"夏季燃烧的植物会释放出大量的烟雾。意在与热浪相互作用。", true),
		BinaryConfig("maxtempdamage", "极端温度的最大生命值损害", "过冷和过热会在短暂延迟后造成血量上限值伤害。",true),
	...
	------------------------------
		...
		BinaryConfig("trapdoorspiders", "活动石板蜘蛛", "草原上的草里，萌新杀手，有蜘蛛战士的效果",true),
		...
		-----------------------------
		Header("Items and Structures","物品和建筑"),
		-----------------------------
		...
		BinaryConfig("scaledchestbuff", "龙鳞宝箱", "启用此选项会将它扩大到25个格子。在已经存在世界中切换选项配置可能会导致崩溃。",true),
		...
		BinaryConfig("passibleimpassibles", "移除碰撞体积", "消除雕像等物品的碰撞，但不包括食人花",true),
		{
			name = "sleepingbuff",
			label = "睡眠的恢复速度",
		...
		BinaryConfig("insul_thermalstone", "暖石修改", "热石现在的隔热效果更差，但是继承了一些衣服的隔热效果。",true),
		...
		Header("Food","食物"),
		-----------------------------
		Header("Crockpot Recipes","烹饪相关"),
		BinaryConfig("newrecipes", "永不妥协专属料理", "永不妥协专属食谱", true),
		BinaryConfig("crockpotmonstmeat", "全新怪物肉", "一个新的怪物肉体系", true),
		BinaryConfig("generalcrockblocker", "填充物削弱", "如树枝，冰，蝴蝶翅膀，和其他不可食用的东西会导致失败料理",true),
		BinaryConfig("icecrockblocker", "雪糕", "雪糕可以防止大量使用冰，特别是在不需要冰的炖菜中。",true),

		Header("Crockpot Food Tweaks","食物调整"),
		BinaryConfig("farmfoodredux", "料理属性改动", "改变了一些料理的三维恢复", true),
		BinaryConfig("sr_foodrebalance", "鱼类料理平衡", "调整了几种以鱼为基础的食物的统计数据.",true),
		...
		Header("General Food Tweaks","普通食品调整"),
		BinaryConfig("no_winter_growing", "冬季不生长",
			"使一些农作物，例如海带和石果在冬天无法生长。", true),
		BinaryConfig("monstersmallmeat", "小怪物肉", "像蜘蛛这样的小生物会掉落小怪物而不是怪物肉",true),
		...
		Header("Harder Hounds","更难的猎犬"),
		BinaryConfig("firebitehounds", "火焰伤害", "红色猎犬攻击时会让玩家着火.", true),
		BinaryConfig("frostbitehounds", "冰冻伤害", "蓝色猎犬攻击时会冻结玩家。", true),


		Header("Wave Changes","猎犬攻击频率"),
		BinaryConfig("lategamehoundspread", "降低袭击频率", "启用这个会降低游戏后期的频率，所以猎犬仍然是一个威胁，但不会令人讨厌。",true),
		...
		Header("New Depths Worms","新的深渊蠕虫"),
		...
		BinaryConfig("batspooking", "蝙蝠刷新机制", "再生的时候立刻出现所有的蝙蝠，而不是慢慢出来。",true),


		Header("Spiders","蜘蛛"),
		BinaryConfig("alljumperspiders", "普通蜘蛛跳跃攻击", "正常的蜘蛛跳跃，就像蜘蛛战士一样。",true),
		BinaryConfig("spiderwarriorcounter", "蜘蛛战士反击", "战斗蜘蛛(和深海生物)在受到攻击时会进行反击(同时降低生命值到300)。",true),


		Header("New Ruins Monsters","新的远古怪物"),
		BinaryConfig("trepidations", "远古恐怖巨蛛", "启用这个功能可以让恐惧在废墟的大厅里漫游，寻找心灵脆弱的人。",true),
		BinaryConfig("pawns", "远古发条士兵", "启用此功能可以让小东西在洞穴深处巡逻，引起愚蠢和迷失者不必要的注意。",true),
		...
		BinaryConfig("mermtweaks", "人鱼调整", "- 普通人鱼现在可以在冬天离开家园，人鱼守卫重生速度较慢。",true),
		...
		Header("Bosses","BOSS"),
		-----------------------------
		Header("Additional Seasonal Giants","额外的四季BOSS"),
		...
		BinaryConfig("wiltfly", "时令龙蝇", "龙蝇现在将在夏天离开她的地盘，攻击玩家，类似于单机版巨人国DLC的龙蝇。",true),
		{
			name = "wiltfly_spawn",
			label = "时令龙蝇出现时间",
			hover = "时令龙蝇会出现的时间",
		...
		-----------------------------
		Header("> Super Sekrit Settings <","> 超级保密设置 <"),
		-----------------------------
		Header("PROCEED WITH CARE","小心行事"),
